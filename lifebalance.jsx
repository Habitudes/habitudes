...
const CATS = [
  { id:"spirit",   label:"Spirit",   icon:"â¦", desc:"Inner life, prayer, presence",                               color:"#F59E0B" },
  { id:"body",     label:"Body",     icon:"â", desc:"Movement, exercise, physical care",                          color:"#10B981" },
  { id:"vocation", label:"Vocation", icon:"â¬¡", desc:"Work, craft, focus, contribution",                           color:"#3B82F6" },
  { id:"bond",     label:"Bond",     icon:"â", desc:"Connection, attentiveness, your closest person",             color:"#EC4899" },
  { id:"shape",    label:"Shape",    icon:"â¬", desc:"Art, learning, hobbies â the form you're becoming",          color:"#8B5CF6" },
];
const SCORES = [-2,-1,0,1,2];
const S_SHORT = {"-2":"â2","-1":"â1","0":"0","1":"+1","2":"+2"};
const S_LABEL = {"-2":"Neglected","-1":"Below par","0":"Neutral","1":"Good","2":"Flourished"};

// âââ PASSPHRASE âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ SCORE UTILS âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function App(){
  ...
  // ââ Reminder check: fire if time has passed today and not yet fired ââ
  useEffect(()=>{
    ...
    const check = ()=>{
      ...
      if(!alreadyFired && now >= fireTime && Notification.permission==="granted"){
        new Notification("habÂ·iÂ·tÅ«des", {
  ...
  if(screen==="loading") return(
    <div style={{minHeight:"100vh",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FF}}>
      <div style={{color:"#AAA",fontSize:13}}>Loadingâ¦</div>
    </div>
  );
      ...
      <div style={{marginTop:48,display:"flex",gap:24,flexWrap:"wrap",justifyContent:"center"}}>
        {["No email required","Completely private","Your key, your data"].map(t=>(
          <div key={t} style={{display:"flex",alignItems:"center",gap:6,color:"#AAA",fontSize:12}}>
            <span style={{color:GREEN,fontWeight:700}}>â</span> {t}
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{minHeight:"100vh",background:"#fff",fontFamily:FF,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px"}}>
      <div style={{width:"100%",maxWidth:360}}>
        <div style={{marginBottom:32,textAlign:"center"}}>
          ...
          <p style={{margin:0,color:"#777",fontSize:14,lineHeight:1.6}}>This passphrase is your anonymous account.<br/>Write it down â it can't be recovered.</p>
        </div>
        ...
        <div style={{background:"#FFFBEB",border:"1.5px solid #FCD34D",borderRadius:10,padding:"14px 16px",marginBottom:24,display:"flex",gap:10}}>
          <div style={{fontSize:16,flexShrink:0}}>â ï¸</div>
        ...
        <Btn onClick={confirmNew} full style={{marginBottom:10}}>I've saved it â let's begin</Btn>
        <Btn onClick={()=>setScreen("splash")} variant="ghost" full>â Back</Btn>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#fff",fontFamily:FF,color:"#000",paddingBottom:80}}>
      ...
      {isDemo&&(
        <div style={{background:GREEN,color:"#fff",textAlign:"center",padding:"8px 16px",fontSize:12,fontWeight:500}}>
          ð You're viewing a demo journal â&nbsp;
          ...
          <div style={{paddingTop:28}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              ...
              <span style={{fontSize:12,color:"#AAA",flexShrink:0,minWidth:54}}>
                {doneCount===5?"Complete â":`${doneCount} / 5`}
              </span>
            </div>
              ...
              <>
                ...
                <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:24,marginBottom:8}}>
                  {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=>{
                    const checked=Object.keys(entries).some(k=>DAYS[new Date(k).getDay()]===d&&entries[k]);
                    return <div key={d} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                      <div style={{width:30,height:30,borderRadius:"50%",border:"2px solid "+(checked?"#1a6b3c":"#ccc"),background:checked?"#e8f5ee":"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:checked?"#1a6b3c":"#bbb",fontWeight:700}}>
                        {checked?"â":""}
                      </div>
                      <span style={{fontSize:10,color:"#888"}}>{d}</span>
                    </div>;
                  })}
                </div>
                <div style={{marginTop:8}}>
                  <Btn onClick={saveEntry} disabled={!allScored} full style={{padding:"15px",fontSize:14,background:saved?"#1a6b3c":undefined,transition:"background 0.3s"}}>
                    {saved?`â  Recorded ${DAYS[new Date(date).getDay()]}`:"Record this day"}
                  </Btn>
                </div>
                {saved&&<div style={{marginTop:16,marginBottom:4,padding:"12px 16px",background:"#f0f8f4",borderRadius:12,textAlign:"center",fontSize:13,color:"#2d6a4f",fontStyle:"italic"}}>
                  âSmall steps every day lead to extraordinary change.â
                </div>}
              </>
            )}
            ...
            <div style={{marginTop:10,background:"#F7F7F7",borderRadius:12,padding:"14px 18px",display:"flex",gap:12,alignItems:"flex-start"}}>
              <div style={{fontSize:18,flexShrink:0}}>ð±</div>
              <div>
                <div style={{fontSize:12,fontWeight:600,color:"#000",marginBottom:3}}>Add to Home Screen</div>
                <div style={{fontSize:12,color:"#777",lineHeight:1.55}}>Safari â Share â "Add to Home Screen." Enables daily reminders.</div>
              </div>
            </div>
          </div>
        )}
...
// âââ ABOUT VIEW ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
const FAQ_SECTIONS = [
  {
    title: "Privacy & Data Security",
    items: [
      { q: "Is my data safe?", a: "There's nothing to protect, honestly. The app never asks for your name, email, or anything about you. You get three random words as your key â that's it. Your entries live on your device. Nobody can see them." },
      { q: "How do I know you're not tracking me?", a: "The app has no backend, no database, no server that stores anything about you. It's a static web app â like a document that runs in your browser. Your journal lives in your phone's local storage, tied to your passphrase. There's nowhere for data to go." },
      ...
      { q: "What happens if I lose my three words?", a: "The journal is gone â there's no recovery. That's a feature, not a bug. It means there's no back door, no \"forgot my password\" flow. You are the only key." },
      { q: "Do you use any analytics?", a: "Vercel (the hosting platform) counts total page visits â just a number, like a hit counter. No personal data, no behavior tracking, no cookies." },
    ]
  },
  {
    title: "How It Works",
    items: [
      { q: "What are the five dimensions?", a: null, list: [
        { label: "Spirit", desc: "Are you growing in your faith journey? Your inner life, sense of meaning, and connection to God.", color: "#F59E0B", icon: "â¦" },
        { label: "Body", desc: "Are you taking care of the vessel? Physical health, sleep, energy, and movement.", color: "#10B981", icon: "â" },
        { label: "Vocation", desc: "Are you showing up with purpose? Your work, contribution, and sense of calling.", color: "#3B82F6", icon: "â¬¡" },
        { label: "Bond", desc: "Are you present in your closest relationship? Your partner, family, and deepest connection.", color: "#EC4899", icon: "â" },
        { label: "Shape", desc: "Are you becoming who you want to be? Character, growth, and the direction of your life.", color: "#8B5CF6", icon: "â¬" },
      ]},
      { q: "What do the scores mean?", a: null, scores: true },
      { q: "What is the 'balance shape'?", a: "Your scores over time form a pentagon on the radar chart. A regular pentagon â five equal sides â represents harmony across all dimensions. Most people's shapes are lopsided in interesting ways. That's the insight." },
    ]
  },
  {
    title: "Getting Started",
    items: [
      { q: "How do I start?", a: "Go to habitudes.app. Tap 'Create my journal.' You'll receive three random words â write them down somewhere safe. That's your key. Then score your day." },
      { q: "Can I add it to my home screen?", a: "Yes. On iPhone, open habitudes.app in Safari, tap the Share button, then 'Add to Home Screen.' It'll behave like a native app â no browser bar, full screen." },
  ...
  },
  {
    title: "Technical",
    items: [
      { q: "What's the architecture? (for tech folks)", a: "Pure client-side React app hosted on Vercel's CDN. No API calls, no database, no authentication. localStorage keyed to a passphrase hash. Vercel analytics sees traffic counts only â no personal data." },
    ...
    <div style={{
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"0 20px"}}>
        ...
        <div style={{marginBottom:36}}>
          ...
          {[
            "Time moves fast. Years have a way of slipping by before we notice that something important quietly stopped getting our attention â a relationship, our health, our faith, our sense of purpose.",
            "I built Habitudes because I wanted a simple, honest way to ask myself each evening: how did I actually show up today? Not a productivity tracker or a goal system â just five quiet questions about the five things that matter most to me.",
            "Over time, the scores draw a shape. That shape reveals something a journal entry can't â the pattern. Where you're flourishing. Where you're drifting. The small, consistent choices that become who you are.",
            "This is a tool for anyone who believes that growth is intentional, that becoming takes attention, and that the examined life â even in sixty seconds â is worth living.",
          ].map((para, i) => (
            <p key={i} style={{fontSize:14,color:"#444",lineHeight:1.75,margin:"0 0 14px",fontStyle: i===3 ? "italic" : "normal"}}>
              {para}
            </p>
          ))}
          <p style={{fontSize:13,color:"#AAA",margin:"16px 0 0"}}>â David Bishop</p>
        </div>
...
// âââ REMINDER CARD âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function ReminderCard({on, time, perm, onChange}){
  ...
  return(
    <div style={{marginTop:10,background:"#F7F7F7",borderRadius:12,padding:"16px 18px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom: on ? 12 : 0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            {blocked
              ? <div style={{fontSize:11,color:"#CC4400",marginTop:2}}>Notifications blocked â enable in browser settings</div>
      ...
      {/* Time picker â only shown when on */}
      {on&&(
...
// âââ SCORE ROW âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ CHART VIEW ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
        ...
        {avgs.map(c=>(
          <div key={c.id} style={{background:"#fff",border:`1.5px solid ${c.avg!==null?c.color+"40":"#EEE"}`,borderRadius:12,padding:"14px 6px",textAlign:"center"}}>
            ...
            <div style={{fontSize:20,fontWeight:700,color:c.avg!==null?scoreColor(c.avg):"#DDD"}}>
              {c.avg!==null?(c.avg>0?`+${c.avg}`:c.avg):"â"}
            </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={lineData} margin={{top:4,right:14,left:-22,bottom:4}}>
            ...
            <Tooltip
              ...
              formatter={(v,name)=>[v!==null?(v>0?`+${v}`:v):"â",name]}
            />
